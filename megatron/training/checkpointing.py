...
def load_args_from_checkpoint(args, load_arg='load', checkpointing_context=None):
    ...
    # MLA / DSv4 hybrid attention args.  These are structural checkpoint
    # properties, so they must be restored before model construction when
    # --use-checkpoint-args is used for validation or inference.
    ...
    _set_arg('rope_type', force=True)
    _set_arg('rotary_scaling_factor', force=True)
    _set_arg('original_max_position_embeddings', force=True)
    _set_arg('beta_fast', force=True)
    _set_arg('beta_slow', force=True)
    _set_arg('mscale', force=True)
    _set_arg('mscale_all_dim', force=True)
    _set_arg('o_groups', force=True)
    _set_arg('o_lora_rank', force=True)
    _set_arg('cache_mla_latents', force=True)
    _set_arg('mla_down_proj_fusion', force=True)
    ...
    _set_arg('csa_window_size', force=True)
    _set_arg('csa_compress_ratios', force=True)
    _set_arg('csa_compress_rotary_base', force=True)
    _set_arg('csa_dense_mode', force=True)
    _set_arg('enable_hyper_connections', force=True)
    _set_arg('num_residual_streams', force=True)
    _set_arg('mhc_sinkhorn_iterations', force=True)
    _set_arg('mhc_init_gating_factor', force=True)
    _set_arg('mhc_recompute_layer_num', force=True)
    ...
    _set_arg('moe_shared_expert_gate', force=True)
    _set_arg('moe_shared_expert_overlap', force=True)
    _set_arg('moe_apply_probs_on_input', force=True)
