    ...
    *,
    ...
    with _cp_layout_nvtx_range("cp_layout/thd/route"):
        ...
        zigzag_index, zigzag_split_sizes = _build_thd_layout_side_route(
            zigzag_segments_by_rank[cp_rank],
            contiguous_segments_by_rank,
            device=device,
        )
        contiguous_index, contiguous_split_sizes = _build_thd_layout_side_route(
            contiguous_segments_by_rank[cp_rank],
            zigzag_segments_by_rank,
            device=device,
        )
...
def get_thd_cp_partition_route(
    ...
    if source_partition_mode == target_partition_mode:
        return None
    if (
        source_partition_mode not in ("zigzag", "contiguous")
        or target_partition_mode not in ("zigzag", "contiguous")
    ):
        raise ValueError(
