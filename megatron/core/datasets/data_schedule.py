...
def get_batch_on_this_rank_for_sequence_packing(
    ...
    if is_first_or_last_stage or mtp_on_this_rank:
        if is_tp_rank_0:
            # Use whichever local data field is available after CP slicing.
            # First stage has tokens, last stage has labels/loss_mask, and
            # tensor truthiness cannot be used for fallback checks.
            for _key in ('tokens', 'labels', 'loss_mask', 'position_ids'):
                _data_field = batch.get(_key)
                if _data_field is not None:
                    break
            else:
                raise ValueError("Cannot determine total_tokens: no local data field found")
