...
class GatedDeltaNet(MegatronModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.cp_size > 1:
            assert config.cp_partition_layout == "contiguous", (
                "GatedDeltaNet with context parallelism requires "
    ...
    def forward(
        ...
        **kwargs,
        ...
        # CP All to All: CP to HP. GDN requires contiguous CP layout, so A2A CP
        # must not undo attention workload balancing that was never applied.
        if packed_seq_params is not None and packed_seq_params.qkv_format == 'thd':
        ...
        # CP all to all: HP to CP. Keep the contiguous CP layout instead of
        # redoing attention workload balancing for zigzag CP.
        if packed_seq_params is not None and packed_seq_params.qkv_format == 'thd':
