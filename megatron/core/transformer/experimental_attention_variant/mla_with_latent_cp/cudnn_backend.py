...
class _CudnnRecomputedPhaseFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        *projection_parameters: Tensor,
        ...
        ctx.execution_stream_id = (
            None if execution_stream is None else int(execution_stream.cuda_stream)
        )
        return output, lse
    ...
    def backward(
        ...
        if query.is_cuda:
            ...
            _require(
                int(backward_stream.cuda_stream) == ctx.execution_stream_id,
                "latent-KV backward must remain on its canonical phase stream",
            )
        if grad_output is None:
            grad_output = torch.zeros_like(raw_output, dtype=torch.float32)
        if grad_lse is None:
...
def _allocate_phase_stream_pair(
    ...
    """Allocate the alternate phase stream before reserving the next connection.

    Projection now runs on the canonical phase streams. Allocating attention first preserves its
    established connection placement, while keeping the following default-priority stream alive
    advances downstream ring-stream placement without inserting work or synchronization.
    """
