...
Historical results below were recorded on September 9, 2026 with
FLA / fla-core 0.5.1 and nvidia-resiliency-ext 0.6.0. Internal infrastructure,
container aliases, and artifact locations are omitted from this public note.
These records do not validate the current rebased integration.

- Configuration validation: 43 tests passed.
- Hybrid validation: dense no-recompute eager/compile/FLA
...
- GPT validation: all seven non-fused scopes transferred
...
- Fused-group, expanded MTP, and recompute combinations have not completed the
  ...
  in the historical records; they are not passing runtime evidence.
...
of the MTP implementation branch. GPU execution remains pending. The wider
support implementation remains unfinished.
...
`use_transformer_engine_op_fuser=True` alone is not runtime evidence. In the
integration target, an unsupported `_is_fused_impl_supported()` result raises
an assertion when the fuser is requested. Tests must confirm actual fused-stack
execution and retain evidence of the grouped MLP kernels.
