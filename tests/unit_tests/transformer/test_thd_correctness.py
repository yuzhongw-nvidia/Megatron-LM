...
def test_mixed_gdn_gqa_model_cp_correctness(model_type, dynamic_context_parallel):
    ...
    try:
        reference_dp_cp_group = parallel_state.get_data_parallel_group(
        ...
        reference_batch, reference_packed_seq_params = _prepare_mixed_model_batch(
            reference_seq_indices,
            reference_cp_group,
            reference_config,
            reference_model.vocab_size,
        )
        reference_stats = _run_mixed_model(
            reference_model,
            reference_batch,
            reference_packed_seq_params,
            reference_dp_cp_group,
        )
    ...
    try:
        candidate_dp_cp_group = parallel_state.get_data_parallel_group(
            with_context_parallel=True
        )
        ...
        reference_loss, reference_mtp_loss, reference_grad_names, reference_grads = (
            reference_stats
        )
