...
def test_cudnn_phase_stream_pair_reserves_connection_before_attention(monkeypatch):
    """The alternate phase stream must follow one persistent connection reservation."""
    ...
    assert allocations == [reservation, attention]
    assert reservation is not attention
    assert reservation.device == attention.device == 3
    assert reservation.ordinal == 0
    assert attention.ordinal == 1
