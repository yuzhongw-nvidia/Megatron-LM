...
def test_split_extra_state_preserves_empty_fp8_state_for_each_gemm():
    module = _grouped_linear_stub(num_gemms=3)
    ...
    splits = module._split_extra_state(empty_state)

    assert len(splits) == module.num_gemms
    assert all(split is empty_state for split in splits)
